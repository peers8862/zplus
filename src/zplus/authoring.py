...
def write_type(name, label, folder, sections, template=None):
    """Write a type (type.toml + template.md) into the user library. Returns its dir."""
    template = template or f"{name}.md"
    d = paths.user_type_dir(name)
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, "type.toml"), "w", encoding="utf-8") as f:
        f.write(type_fragment(name, label, folder, template, sections))
